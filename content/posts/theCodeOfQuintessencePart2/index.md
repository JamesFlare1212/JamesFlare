...
新卷新气象，我们希望在第贰卷为大家展示与第壹卷(与其延伸)完全不同的可能性，因此除了精美的画面外，本卷将更注重于展示能形成独特画风的组合与新描绘方法。
...
各位好，这里是编辑部全员，很高兴与大家再次相会！元素法典第一卷与一点五卷的发布带来了相当大的热度和好评。社区投稿也因此变得更为踊跃；在发布前，我们处理的稿件数量竟然了超过一点五卷一个数量级！非常感谢大家的热情支持！这次投稿的质量奇迹般地高，我们看得非常开心，全员无不惊叹大家的创造力！

由于选中的稿件实在过多，因此本卷被分为了三章，第一章将着重关注风格化作品，第二章将聚焦于描绘物体或风景，而第三章则是大家喜闻乐见的经典高质量展示！那么，视觉盛宴之旅，现在开始吧！

## 风格专章

本章特别收录那些专精于介绍特殊画风或特殊构图形式的作品，它们尤其擅长令人耳目一新。某些引入了较新风格但仅将其作为辅助元素的作品不在此列。
...
“希望更多国风方面的魔法出现吧，也希望有大佬来训练下AI的国风画风”
...
{{< image src="waterColourPortraitCode.webp" width="350px" caption="人像水墨法" >}}
...
> 
> 可以加入 `(female focus on)`, `(lens 135mm f1.8)`（据说有摄像机的原理）
...
> Steps: 50, Sampler: Euler, CFG scale: 7, Size: 1280x832, Model hash: e6e8e1fc, Clip skip: 2
...
“目前已经稳定下来了，可以自定义最后的视角与环境获得不同的效果图。”
...
> 
> `leggings` 与 `cat tail` 使用弱权重，在附加“物”且还是最末的位置容许被挤占。`legging` 用于在 AI 想拍腿的时候提供黑色袜子而非光腿，而 `0.85` 是它不会显著影响构图的阈值。`cat tail` 起上文的反作用，弱化前置树的叶能弱化整棵树的存在感，让 `cat girl` 自带的猫尾属性不污染围巾——同理还有下文的 `fireflies`。
> 
...
{{< image src="plantCode.webp" width="250px" caption="林奈法" >}}
...
> 
> 最前面使用常見的品質類咒語，接著指定畫面主題（貓娘在花園裡），並且對畫面要素做出指定（花、蝴蝶）。使用特殊的混色法進行畫面調色，例如 `silver hair`, `blue hair` 的混用，在使用 DDIM （其他 sampler 的效果需測試）的時候，高機率出現髮色銀色但整體畫面變藍的趨勢。本咒語就利用了此特性進行畫面調色。


反面 tag：

> (3 cat ear), (poorly drawn hands), (poorly drawn face), (((fat))), ((cropped)), ((fused fingers)), ((too many fingers)), (malformed limbs), (((bad anatomy))), ((ugly)), out of frame, blurry, gross propotions, distorted face, distorted body, ((distorted fingers)), missing leg, more than 2 leg, more than 2 feet, more than 2 arms, text, ui, signature, icon, watermark, misplaced limbs, leg too big, leg too small, fused hands, fused arms, distorted backgroud, fused buildings, ((finger too short)), more than 1 right hand, more than 1 left hand, wrong direction of limbs, wrong direction of legs, wrong direction of feet, misplaced facial features, unbalanced facial features, ((body too long)), ((arm too short)), (poorly drawn joint), misplaced joint, wrong joint angle, disappearing legs, disappearing arms, disappearing limbs, ((fused limbs)), ((fused thighs)), thumb too long, fingers too long, missing hands, missing arms, no face, wrong angle of upper and lower body, ((misplaced animal tail)), ((malformed animal tail)), poorly drawn animal tail, less than 2 ears, ((more than 2 ears)), ((more than 2 animal ears)), ((one arm longer than another)), ((missing palm)), ((palm disappeared)), sharp fingers, ((two eyes in different size)), hair fused with items, weird ears, weird animal ears, weird fingers, weird hands, weird limbs, weird arms, weird legs, weird thighs, thin wrist, more than 2 eyes, malformed eyes, misplaced eyes, strange animal
> 
> 本反咒並非複製貼上或隨意堆疊，扣除最前面的幾個常見反咒（`cropped`, `blurry`等）之外，其他皆是為了畫面元素做調整。首先四肢、五官的各類型反咒（`malformed`, `misplaced`, `weird`等），然後對耳朵做特殊的指定(不然極其容易生成三隻、四隻耳朵的貓娘)，最後指定 `strange animal` 避免生成奇怪的貓。（生成獸耳娘時有用）
...
> ESDN: 17415
> 
> 除了 DDIM 外，Euler、LMS、DPM2 a Karras 都可以獲得不錯的效果，只是 step 需要適當的調低。
> 
> 若1024寬的圖片解析度太低，推薦使用 hires fix 功能處理，也可以有效的避免過於鮮豔的問題。
...
{{< image src="carryLiveCode.webp" width="450px" caption="樱乐会" >}}

作者：某驾校校长

“知道你在台下哦 这首歌 是唱给你的专属love！”

正面 tag：

> ((masterpiece)), (((best quality))), ((ultra-detailed)), ((illustration)), A lot of waving glow sticks, Stage, Concert, (solo), 1 girl, ((singing)), headset, (leaning_forward:1.2), (arms_behind_back), ((extremely_detailed_eyes_and_face)), colorful, Tokyo Dome, ray tracing, (disheveled hair), cherry_blossoms, petals, Flying notes

反面 tag：

> An incomplete face, A contorted face, hand, lowres, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry, missing fingers, bad hands, long neck, Humpbacked, extra legs

参数：

> Steps: 48, Sampler: Euler, CFG scale: 13, Size: 1344x832, Model hash: e6e8e1fc, Clip skip: 2

### 瓶中法

{{< image src="bottleWorldCode.webp" width="250px" caption="瓶中法" >}}
...
> mutated hands and fingers:1.5, long body:1.3, mutation, poorly drawn:1.2, bad shadow, unnatural body, bad breasts, bad face, bad eyes, extra eyes, {{{ugly}}}, {{{duplicate}}}, {{morbid}}, {{mutilated}}, {{{tranny}}}, mutated hands, {{{poorly drawn hands}}}, blurry, {{bad anatomy}}, {{{bad proportions}}}, extra limbs, cloned face, {{{disfigured}}}, {{{more than 2 nipples}}}, {{{{missing arms}}}}, {{{extra legs}}}, mutated hands, {{{{{fused fingers}}}}}, {{{{{too many fingers}}}}}, {{{unclear eyes}}}, lowers, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry, bad feet, {{{{{monotone}}}}}, Abnormal eye size, Abnormal eye distribution, Abnormal eye proportion, Abnormal hands, Abnormal legs, Aabnormal feet, Abnormal fingers, sharp face

参数：

> Steps: 50, Sampler: Euler a, CFG scale: 11, Size: 512x1024, Model hash: 925997e9, Clip skip: 2

### 未名花

{{< image src="unnamedFlowerCode.webp" width="275px" caption="未名花" >}}

作者：まっちゃ (原作者主页：[@maccha3gooo](https://twitter.com/maccha3gooo?s=11&t=Zw8siP-48ZEB16JzabeOsw))
...
{{< image src="unnamedRainCode.webp" width="325px" caption="未名雨" >}}

作者：まっちゃ (原作者主页：[@maccha3gooo](https://twitter.com/maccha3gooo?s=11&t=Zw8siP-48ZEB16JzabeOsw))

正面 tag：

> melting, abstract, splash, hyper detailed, small breast, white hair, short hair with long locks, small breast, messy hair, blue eyes, backlighting, sand, water, black background, dust clodust, water splash, long sleeves, white clothes, splash, oversized clothes

反面 tag：

> {{{{{large breasts}}}}}, {{{{{huge breasts}}}}}, {{{{{worst quality}}}}}, {{{{{fat}}}}}, child, {{{{{low-teen}}}}}, masterpiece, black hair, adult, old, phobia, loli, low-teen, blush, fate, realistic, Smrealist, clothes, nsfw, masterpiece, a, naturalistic, hair, hair, sand, dress, hatching, absolutely long hair, nsfw, owres, bad anatomy, bad hands, text, error, missing fingers, extra digit, liquid tongue, long neck, fused ears, bad ears, poorly drawn ears, extra ears, liquid ears, heavy ears, missing ears, fused animal ears, bad animal ears, poorly drawn animal ears, extra animal ears, liquid animal ears, heavy animal ears, missing animal ears, bad hairs, poorly drawn hairs, fused hairs, bad face, fused face, poorly drawn face, cloned face, big face, long face, bad eyes, fused eyes poorly drawn eyes, extra eyes
...
{{< image src="Code.webp" width="450px" caption="" >}}
